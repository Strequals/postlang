...
static const std::regex BYTE_TYPE_N("[0-9]+b");
static const std::regex INT_TYPE_G("[-|+]?[0-9]+");
static const std::regex INT_TYPE("[-|+]?[0-9]+i");
static const std::regex LONG_TYPE("[-|+]?[0-9]+l");
static const std::regex FLOAT_TYPE_G("[-|+]?[0-9]+[.]?[0-9]+");
static const std::regex FLOAT_TYPE("[-|+]?[0-9]+[.]?[0-9]+f");
static const std::regex DOUBLE_TYPE("[-|+]?[0-9]+[.]?[0-9]+d");
...
void compile_token(std::vector<byte> &compiled, function_map &functions, std::vector<InvokePlaceholder> &invokes, variable_map &variables, stack_location &stack_loc, std::string &token) {
	...
	if (std::regex_match(token, BYTE_TYPE_C)) {
		...
		std::cout << "byte token: " << token << std::endl;
		compiled.push_back(PUSH);
		compiled.push_back((byte) std::stoi(token.substr(0,token.length()-1)));
		stack_loc++;
		...
		std::cout << "int token: " << token << std::endl;
		errno = 0;
		long int_value = std::stol(token.substr(0,token.length()-1));
		if (errno == ERANGE) {
			std::string s = "token " + token + " out of int range";
			handle_error(s);
		}
		...
		errno = 0;
		long long int long_value = std::stoll(token.substr(0,token.length()-1));
		if (errno == ERANGE) {
		...
		std::cout << "float token: " << token << std::endl;
		errno = 0;
		float float_value = std::stof(token.substr(0,token.length()-1));
		if (errno == ERANGE) {
			std::string s = "token " + token + " out of float range";
			handle_error(s);
		}
		...
		errno = 0;
		double double_value = std::stod(token.substr(0,token.length()-1));
		if (errno == ERANGE) {
		...
		if (f_map.second) {
			...
			stack_loc += fd.return_len - fd.param_len;
			return;
		}
...
FunctionData precompile_function(function_map functions, FunctionSource &f_source) {
	std::cout << "precompiling " << f_source.name << ":\n" << f_source.params << std::endl << f_source.returns << std::endl;
	long parameter_size = byte_count(f_source.params);
	long return_size = byte_count(f_source.returns);
	FunctionData fd;
	fd.param_len = parameter_size;
	fd.return_len = return_size;
	std::cout << "function " << f_source.name << " takes " << parameter_size << " returns " << return_size << std::endl;
	return fd;
}
...
FunctionData compile_function(std::vector<byte> &compiled, function_map functions, std::vector<InvokePlaceholder> &invokes, FunctionSource &f_source) {
	...
	std::cout << f_source.returns << std::endl;
	std::cout << f_source.body << std::endl;
	variable_map variables;

	long parameter_size = parse_parameters(variables, f_source.params);
	long return_size = byte_count(f_source.returns);
	...
	for (unsigned int i = 0; i < source.length(); i++) {
		...
		if (processToken) {
			...
			if (token == "let") {
				...
				std::string return_source = source.substr(i,end-i) + "\n";
				std::cout << "ret source: " << return_source;
...
void compile(std::vector<byte>	&compiled, std::string &source) {
	...
	while ((function_define_location = source.find("define ", pass_location)) != std::string::npos) {
		...
		std::pair<int,int> return_brackets = matching_brackets('(',')',source,close_param_location);
		int open_return_location = return_brackets.first;
		int close_return_location = return_brackets.second;

		std::pair<int,int> source_brackets = matching_brackets('{','}',source,close_return_location);
		...
		f_source.returns = source.substr(open_return_location+1,close_return_location-open_return_location-1);
		...
		FunctionData fd_pre = precompile_function(functions, f_source);

		functions[f_source.name] = fd_pre;
	...
	if (f_sources.size() > 0) {
		counter x = compiled.size();
		compiled[1] = (x >> 24) & 0xFF;
		compiled[2] = (x >> 16) & 0xFF;
		compiled[3] = (x >> 8) & 0xFF;
		compiled[4] = (x) & 0xFF;
	}
